...
```mermaid
graph LR
    A[VIDEO FILES] --> P[검출 + 트래킹 + Re-ID + 매칭]
    P --> U[UI 출력]
```
...
**비즈니스 메시지**: 사후 분석·보안 조사·공간 이용 패턴 파악

#### 시나리오 B — 관심 인물 탐색 (오프라인)

**컨셉**: "녹화 영상에서 특정 인물이 어디에 등장했는지 찾아드립니다"

**데모 흐름**
1. 영상 폴더 선택
2. 관심 인물 지정 (썸네일 클릭 또는 ID 입력)
3. 전체 영상에서 해당 인물 등장 구간 자동 탐색
4. 등장 시각, 카메라, 유사도 + 썸네일 클립 출력
...
| 순위 | 항목 | 내용 |
| :--- | :--- | :--- |
| 1 | 영상 처리 파이프라인 | 영상 파일 → 검출 → 트래킹 → Re-ID → 매칭 |
| 2 | UI / 시각화 | 동선 타임라인 / 등장 구간 썸네일 출력 |
| 3 | 데모용 영상 준비 | 미학습 슬롯 사용 (t1/t2/t4 권장) |

#### 시각 정보 추출 방식

영상 파일만 전달받을 경우 **파일명 → `config.yaml` 슬롯 매핑**으로 절대 시각을 자동 계산.

**계산식**:
```
슬롯 시작 시각 (config.yaml)
  + frame_index / src_fps
= 절대 시각
```

**예시** (`cam2_t9.avi`, frame 35148):
```
t9 시작 = 16:00:00
35148 / 24fps = 1464.5초 = 24분 24초
→ 절대 시각 16:24:24
```

**config.yaml 슬롯 → 시작 시각 매핑**:

| 슬롯 | 시작 시각 | 파일 예시 |
| :--- | :--- | :--- |
| t1 | 12:00:00 | cam1_t1.avi, cam2_t1.avi |
| t2 | 12:30:00 | cam2_t2.avi, cam3_t2.avi |
| t3 | 13:00:00 | cam1_t3.avi, ... |
| t4 | 13:30:00 | cam1_t4.avi, cam2_t4.avi |
| ... | ... | ... |
| t10 | 16:30:00 | cam1_t10.avi, ... |

→ 파일명만 있으면 별도 입력 없이 타임라인 자동 생성 가능.

#### 미결 결정 사항 (단계 1)

| 항목 | 선택지 |
| :--- | :--- |
| 결과 UI | 터미널 출력 / 웹 대시보드 / OpenCV 윈도우 |
| 영상 입력 | 파일 경로 지정 / 드래그앤드롭 |
| 데모용 영상 슬롯 | t1 (cam1+cam3) / t2 (cam2+cam3) / t4 (cam1+cam2) |
...
```mermaid
graph TB
    CAM_A([cam_A.mp4\n또는 카메라 A]) --> PC1[Ubuntu PC #1\n검출 + 트래킹]
    CAM_B([cam_B.mp4\n또는 카메라 B]) --> PC2[Ubuntu PC #2\n검출 + 트래킹]
    CAM_C([카메라 C]) --> JET[Jetson Orin Nano\n검출 + 트래킹 + Re-ID]
